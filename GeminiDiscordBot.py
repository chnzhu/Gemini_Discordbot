        ...
        async with message.channel.typing():
            ...
            if message.attachments:
                ...
                for attachment in message.attachments:
                    print(
                        f"New Image Message FROM: {message.author.name} : {cleaned_text}"
                    )
                    ...
                    if any(
                        ...
                        print(
                            f"New Text Message FROM: {message.author.name} : {cleaned_text}"
                        )
            ...
            # Not an Image, check for text responses
                ...
                print(
                    f"New Message Message FROM: {message.author.name} : {cleaned_text}"
                )
...
async def ProcessURL(message_str):
    ...
    if is_youtube_url(extract_url(message_str)):
        ...
        return await generate_response_with_text(
            pre_prompt + " " + get_FromVideoID(get_video_id(extract_url(message_str)))
        )
    if extract_url(message_str):
        print("Processing Standards Link")
        return await generate_response_with_text(
            pre_prompt + " " + extract_text_from_url(extract_url(message_str))
        )
    else:
        return "No URL Found"
